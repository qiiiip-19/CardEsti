...
def filterDict2Hist(hist_file, filterDict, encoding):
    """
    A robust version of filterDict2Hist that returns zeros if hist_file is None.
    """
    if hist_file is None:
        # Return a zero array of the expected length for histogram features
        return np.zeros(EXPECTED_HIST_FEATURES_LEN)
    # Call the original filterDict2Hist if hist_file is available
    return filterDict2Hist(hist_file, filterDict, encoding)
